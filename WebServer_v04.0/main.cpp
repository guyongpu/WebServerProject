...
//const string PATH = "/home/yongpu/WebServerProject/WebServer_v4.0/resource/";
const string PATH = "../resource/";

const int TIMER_TIME_OUT = 5000;

extern pthread_mutex_t qlock;
extern struct epoll_event *events;
extern TimerQueue myTimerQueue;    // 时间堆
...
int main() {
    system("pwd");
    handle_for_sigpipe();   // 忽略SIG_IGN信号
    int epoll_fd = epoll_init();
    if (epoll_fd < 0) {
        perror("epoll init failed");
        return 1;
    }
    threadpool *mythreadpool = new threadpool(THREADPOOL_THREAD_NUM, QUEUE_SIZE, 0);
    ...
    while (true) {
        ...
        handle_events(epoll_fd, listen_fd, events, events_num, PATH, mythreadpool);
        /*在每次检查是否存在超时定时器*/
...
// 检查port值，取正确区间范围
    ...
    // 消除bind时"Address already in use"错误,允许端口复用
    int optval = 1;
...
void handle_expired_event() {
    pthread_mutex_lock(&qlock);
    while (!myTimerQueue.is_empty()) {
        mytimer *ptimer_now = myTimerQueue.gettop();
        cout << "ptimer_now->isDeleted() = " << ptimer_now->isDeleted() << endl;
        if (ptimer_now->isDeleted()) {
            cout << "ptimer_now is Deleted" << endl;
            myTimerQueue.deltop();
            delete ptimer_now;
        } else if (ptimer_now->isvalid() == false) {
            cout << "ptimer_now is not valid" << endl;
            myTimerQueue.deltop();
            delete ptimer_now;
        } else {
            break;
