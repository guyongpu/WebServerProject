...
const std::string Epoll::PATH = "../resource/";
...
void Epoll::my_epoll_wait(int listen_fd, int max_events, int timeout) {
    ...
    if (req_data.size() > 0) {
        for (auto &req: req_data)/*将请求都加入到任务队列中*/
        {
...
// 分发处理函数
std::vector<std::shared_ptr<requestData>>
Epoll::getEventsRequest(int listen_fd, int events_num, const std::string path) {
    ...
    for (int i = 0; i < events_num; ++i) {
        ...
        } else if (fd < 3)    /*系统文件描述符，0,1,2分别对应 标准输入、标准输出、标准错误*/
        {
            break;
        } else {
            // 排除错误事件，EPOLLHUP对方关闭
            ...
            std::shared_ptr<requestData> cur_req(fd2req[fd]);/*每个文件描述符都有一个对应的requestData用来存数据*/
