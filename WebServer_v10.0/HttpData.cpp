...
HttpData::HttpData(EventLoop *loop, int connfd) :
        ...
        hState_(H_START),
        keepAlive_(false) {
    // 使用初始化参数列表时，初始化列表中数据成员的初始化顺序应该和在类的声明中给出数据成员的顺序要一致
...
AnalysisState HttpData::analysisRequest() {
    if (method_ == METHOD_POST) {
        ...
        if (fileName_ == "favicon.ico") {
            ...
            header += "Server: Yongpu's Web Server\r\n";
        ...
        //文件路径 = 资源路径 + 文件名
        fileName_ = path_ + fileName_;
//        cout << "this->path_ = " << this->path_ << endl;
//        cout << "fileName_ = " << fileName_ << endl;
...
void HttpData::handleError(int fd, int err_num, string short_msg) {
    ...
    header_buff += "Server: Yongpu's Web Server\r\n";;
    header_buff += "\r\n";
    // 错误处理不考虑writen不完的情况
